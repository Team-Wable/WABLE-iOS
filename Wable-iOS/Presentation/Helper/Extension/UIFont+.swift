...
extension UIFont {
    ...
    /// `Pricedown` 폰트를 반환하는 정적 메서드.
    ///
    /// - Parameter style: `Pricedown` 스타일(enum)
    /// - Returns: 해당 스타일에 맞는 `UIFont` 객체를 반환합니다.
    ///
    /// 사용 예시:
    /// ```swift
    /// let titleFont = UIFont.pricedown(.black)
    /// ```
    static func pricedown(_ style: Pricedown) -> UIFont {
        return UIFont(name: style.fontName, size: style.size) ?? .systemFont(ofSize: style.size)
    }
}
